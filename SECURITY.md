...
Cloud endpoint URLs must use HTTP(S) and must not contain embedded credentials. HTTPS is recommended for any non-local endpoint. Preset providers (Groq and Tencent Cloud) pin their endpoint on the Host side; only the Custom OpenAI-compatible provider accepts a user-supplied endpoint.
