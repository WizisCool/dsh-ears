import { ASR_BACKEND_IDS, CLOUD_ASR_PROVIDER_IDS, DEEPGRAM_ASR_SERVICE_IDS, MAX_CLOUD_API_KEY_LENGTH, MAX_POLISH_PROMPT_LENGTH, SETTINGS_DISPLAY_NAME_IDS, TENCENT_ASR_SERVICE_IDS, WHISPER_ACCELERATION_IDS, WHISPER_MODEL_IDS, isBailianAsrHost, isHttpEndpoint, isValidRecordingLimit } from '../config.js'
...
export function cloudAsrModelField(provider: string): 'cloudAsrGroqModel' | 'cloudAsrDeepgramModel' | 'cloudAsrCustomModel' | 'cloudAsrBailianModel' | 'cloudAsrTencentEngineType' {
...
export function isSettingsFieldInvalid(field: FieldName, text: string): boolean {
  if (field === 'webSpeechLanguage' || field === 'localWhisperLanguage' || field === 'cloudAsrGroqLanguage' || field === 'cloudAsrDeepgramLanguage' || field === 'cloudAsrCustomLanguage' || field === 'cloudAsrBailianLanguage') return false
  ...
  if (field === 'cloudAsrGroqApiKey' || field === 'cloudAsrDeepgramApiKey' || field === 'cloudAsrCustomApiKey' || field === 'cloudAsrBailianApiKey' || field === 'cloudAsrTencentSecretKey') return text.length > MAX_CLOUD_API_KEY_LENGTH
  ...
  if (field === 'cloudAsrGroqModel' || field === 'cloudAsrDeepgramModel' || field === 'cloudAsrCustomModel' || field === 'cloudAsrBailianModel' || field === 'cloudAsrTencentAppId' || field === 'cloudAsrTencentSecretId' || field === 'cloudAsrTencentEngineType') return false
