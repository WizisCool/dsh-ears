      ...
      {activeTab === 'recognition' ? (
        <div id={`${tabsId}-panel-recognition`} role="tabpanel" aria-labelledby={`${tabsId}-tab-recognition`} className={styles.panel}>
          ...
          {state.asrBackend.text === 'cloud-openai' ? state.cloudAsrProvider.text === 'groq' ? <>
            <KeyRow label={t('cloudKey')} hint={t('cloudKeyHint')} configured={state.cloudAsrApiKeyConfigured} disabled={!state.writable} invalid={state.cloudAsrApiKey.invalid} lastSaved={state.lastSaved} onEdit={props.setApiKey} onClear={props.clearApiKey} onSaveNow={props.saveNow} t={t} />
...
function KeyRow({ label, hint, configured, disabled, invalid, lastSaved, onEdit, onClear, onSaveNow, t }: { label: string; hint: string; configured: boolean; disabled: boolean; invalid: boolean; lastSaved: { revision: number; fields: readonly FieldName[] }; onEdit: (text: string) => void; onClear: () => void; onSaveNow: () => void; t: Translate }) {
  const [draft, setDraft] = useState('')
  const [savedFlash, setSavedFlash] = useState(0)
  const revisionRef = useRef(lastSaved.revision)
  useEffect(() => {
    if (lastSaved.revision === revisionRef.current) return
    revisionRef.current = lastSaved.revision
    if (lastSaved.fields.includes('cloudAsrApiKey')) setSavedFlash((value) => value + 1)
  }, [lastSaved])
  useEffect(() => {
    if (savedFlash === 0) return
    const timer = setTimeout(() => setSavedFlash(0), 1500)
    return () => clearTimeout(timer)
  }, [savedFlash])
  const justSaved = savedFlash > 0
  return (
    <RowField label={label} hint={hint} invalid={invalid}>
      <div className={styles.keyControl}>
        <input
          className={`${styles.textInput} ${justSaved ? styles.textInputSaved : ''}`}
          type="password"
          autoComplete="off"
          autoCapitalize="off"
          autoCorrect="off"
        ...
        />
        {justSaved ? <span key={savedFlash} className={styles.savedFlash}>{'✓ '}{t('keySaved')}</span> : null}
        {configured ? <button type="button" className={styles.linkButton} disabled={disabled} onClick={() => {
          setDraft('')
          onClear()
        }}>{t('clearKey')}</button> : null}
      </div>
    </RowField>
  )
}
