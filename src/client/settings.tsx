      ...
      {activeTab === 'recognition' ? (
        <div id={`${tabsId}-panel-recognition`} role="tabpanel" aria-labelledby={`${tabsId}-tab-recognition`} className={styles.panel}>
          ...
          {state.asrBackend.text === 'cloud-openai' ? state.cloudAsrProvider.text === 'groq' ? <>
            <TextRow label={t('cloudEndpoint')} hint={t('cloudEndpointGroqHint')} value={state.cloudAsrEndpointEffective} disabled invalid={false} onChange={() => undefined} />
...
function KeyRow({ label, hint, configured, disabled, invalid, onEdit, onClear, t }: { label: string; hint: string; configured: boolean; disabled: boolean; invalid: boolean; onEdit: (text: string) => void; onClear: () => void; t: Translate }) {
