...
- The 关于 / About tab (D-033) is implemented as the last settings tab: name, installed version, MIT license, dsh compatibility, and a click-only npm `latest` check that never installs. Public npm / repo / tag / Release remain gated until an explicit publish go-ahead.
