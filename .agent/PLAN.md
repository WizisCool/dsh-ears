...
- The 关于 / About tab (D-033) is implemented as the last settings tab: a jumpable GitHub repo row, installed version, MIT license, dsh compatibility, and a click-only npm `latest` check that never installs. Public npm / repo / tag / Release remain gated until an explicit publish go-ahead.
...
| D31 | Flat native General-style rows with per-field auto-save. No card chrome and no Save/Discard footer. Valid drafts flush after 400 ms, on blur, or on unmount; invalid drafts are skipped locally; failed writes do not retry in a loop. No third-party form library (D-025). | Accepted |
| D33 | Last 关于 / About tab: identity rows plus click-only Host npm `latest` check. Notify and copy `dsh plugin --profile web update dsh-ears`. No in-app install. First public release waits for an explicit publish go-ahead. | Accepted |
...
The polishing prompt is a multilingual ASR-editing contract: it removes filler words across languages, repairs likely ASR errors, restores punctuation and typographic spacing, preserves the transcript's original language and technical/code terminology, resolves spoken self-corrections, formats explicit enumerations as lists, and treats transcript text as data rather than instructions. Users may replace it entirely with their own system prompt on the Polishing tab (`polishPrompt`); leaving it blank uses the built-in default, and the host always appends an invisible output-contract guard (return only the polished text) to any custom prompt. The Host bounds both input and streamed output; any timeout, cancellation, route failure, or oversized result falls back to the raw transcript. The runtime prompt may be Chinese; its implementation and tests remain English-documented.
...
- Recognition group: a grouped backend/provider selector (Local: Web Speech / Local Whisper; Cloud providers: Groq / Alibaba Cloud Model Studio / Custom OpenAI-compatible), local Whisper model, per-provider write-only API keys, Groq live model list, Bailian HTTPS host + typed Flash model (DashScope sync, 300-second cap), and a custom transcription endpoint.
- Polishing group: enabled/disabled and a provider/model selector populated from dsh's configured routes. The group's rows appear only after polishing is enabled; a complete provider/model pair activates polishing, and an incomplete pair leaves it dormant (D-024). A custom polish system prompt row lives in the same group: blank = built-in default, non-empty = replaces the default (host appends the output guard), with a live `n/4000` counter, Reset-to-default, and a read-only "View default" expand.

The page keeps the same draft and read-only fallback as the previous card, without card chrome or a Save/Discard footer. Valid drafts auto-save after 400 ms, on text blur, or when the section unmounts (D-031). An invalid draft is skipped and stays local with a red hint; a Host rejection keeps the drafts with a red saveFailed line and does not retry in a loop. Empty text clears a field on flush, and untouched or not-yet-configured fields render quietly (D-024). An incomplete provider/model pair leaves polishing dormant until completed (revised D3). Stale Whisper action responses cannot replace a newer selection, and failed model operations retain a retry action. The first release has no emotion toggle and no plugin-owned LLM credential fields. Cloud ASR keys remain Host-side and separate from polishing; the plugin stores the key in a `role('secret')` settings field whose value never crosses the wire back to the browser.
