...
- Decision: Groq, custom OpenAI-compatible, and Bailian each store a separate `role('secret')` key (`cloudAsrApiKey`, `cloudAsrCustomApiKey`, `cloudAsrBailianApiKey`). Switching providers no longer overwrites another provider's key. Package unreleased; no migration.
