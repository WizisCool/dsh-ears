...
- Decision: replace the 400 ms debounced auto-save with the shipped plugin card's staged-draft model. Edits stage as drafts; the footer gains the card's separator plus always-visible Save and Discard buttons. Save is blocked unless the page is dirty, has no invalid draft, and is idle (label 保存中… while writing); Discard drops every draft and a failed state without confirmation. An invalid draft blocks the whole save and keeps the drafts; a Host-rejected write keeps the drafts with the red saveFailed line. Empty text clears a field on save (endpoint/model/polish fields write `''`); the API key keeps absent=keep semantics with a staged clear action (typing a new key cancels the pending clear), and an emptied recording limit resets to the default. A mid-save edit survives the in-flight write and stays dirty.
