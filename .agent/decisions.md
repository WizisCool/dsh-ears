...
| D-049 | dsh 0.1.2 development line | Accepted. Establishes the next-only 0.3 line; the master 0.2 compatibility policy remains governed by D-035. |
...
## D-048 — Volcengine streaming and recording-file recognition

- Status: accepted (2026-08-29); implementation tracks issue #37.
- Decision: add Volcengine (`volcengine`, the official English brand spelling) as a first-class cloud ASR provider with one provider entry exposing two services, mirroring the Tencent Cloud dual-service pattern (D-041): a `recording-file` service over 录音文件识别标准版 (`POST https://openspeech.bytedance.com/api/v3/auc/bigmodel/submit`, polled through `POST .../api/v3/auc/bigmodel/query` with the submitting `X-Api-Request-Id` doubling as the task id) and a `realtime` service over 单向流式语音识别 (`wss://openspeech.bytedance.com/api/v3/sauc/bigmodel_nostream`). The one-way streaming interface is chosen over the bidirectional interface because Volcengine positions it for voice-input scenarios and reports higher whole-sentence accuracy, which matches the manual-send dictation flow.
- Decision: authentication uses the new speech console `X-Api-Key` (API Key 管理) only. Legacy console auth (`X-Api-App-Key` + `X-Api-Access-Key`) is out of scope, and the README must state that explicitly. The provider stores one write-only `role('secret')` key under `cloudAsr.volcengine.apiKey` (flat wire `cloudAsrVolcengineApiKey`, masked as `cloudAsrVolcengineApiKeyConfigured`).
- Decision: the selectable model version is the resource id while `request.model_name` stays the fixed `bigmodel`. Two model fields are gated by `visibleWhen` on the service field so each service validates its own allowed values: the realtime field accepts `volc.seedasr.sauc.duration` and `volc.bigasr.sauc.duration` (default `volc.seedasr.sauc.duration`); the recording-file field accepts `volc.seedasr.auc` and `volc.bigasr.auc` (default `volc.seedasr.auc`). The concurrency-billed realtime variants are not exposed: single-user dictation has no concurrency pool to draw on. Option labels stay friendly and localized while the raw resource id is surfaced through the shared tooltip primitive on hover, a registry-level behavior any enumerated provider field reuses. Wire fields are `cloudAsrVolcengineService`, `cloudAsrVolcengineRealtimeModel`, and `cloudAsrVolcengineRecordingModel`.
- Decision: recognition language follows D-042 with one shared free-form field `cloudAsrVolcengineLanguage` (storage `cloudAsr.volcengine.language`). An empty value omits `audio.language`, which the server defines as 中文, 英文, and 沪/闽/川/陕/粤 dialects; non-empty values pass through (27 documented codes such as `zh-CN`, `en-US`, `yue-CN`). `enable_auto_lang` is not exposed in v1: it is mutually exclusive with `corpus` and silently changes recognition behavior, while the empty default already covers the plugin's primary languages.
- Decision: the realtime adapter streams raw 16 kHz mono PCM16 chunks through the binary WebSocket framing — a 4-byte header (protocol version and header size, message type and flags, JSON serialization plus gzip compression), a gzip-compressed JSON full request, positive-sequence audio-only chunks, and a final chunk flagged `NEG_WITH_SEQUENCE` with a negative sequence. It sends `show_utterances=true` and maps the accumulated definite utterances from each server response onto the existing `sendRealtimeAudio` `{text, final}` channel, so the composer draft updates when the gateway reports text. `enable_itn=true`, `enable_punc=true`, `enable_ddc=false`, and `result_type=full` are fixed in v1 with no settings surface. The whole-sentence result arrives with `is_last_package`, after which the server closes the socket with close code 1000, so a server close after the last package is a successful finish rather than an incomplete stream. Live verification on 2026-08-29 confirmed the framing, the silence end marker, and that the one-way gateway keeps intermediate `result.text` empty until the final package — the transcript lands on finish, matching the interface's non-realtime positioning.
