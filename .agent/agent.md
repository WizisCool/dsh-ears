...
- Unfinished: D-018 (recording-settings snapshot versus locking) remains open; the Groq preset still needs a real rc.6 Web smoke (Host-side changes require a `dsh web` restart outside this session) and a live Groq `zh` transcription smoke with a real key; Windows smoke remains pending. Live symptom confirmed 2026-08-16: on the current `dsh web` (process started 20:55, pre-D-023 Host in memory, plugin installed via `link:` to this repo), the rebuilt client saves fail with the red "保存失败" footer and the amber load notice — a wire-mismatch artifact reproduced by parsing a `cloudAsrApiKey` patch with the old patch schema (the key field is stripped and the old view fails the new client view schema). Restarting `dsh web` (outside this session) plus a browser refresh resolves it; no code defect found.
- Blocked: none.
