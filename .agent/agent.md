...
- Latest commit: pending About repo row and thesvg GitHub mark on `master`.
...
- Completed: last settings tab **关于 / About** (D-033). Identity rows have no hint copy. The first row is **仓库** with the thesvg.org `Github` mark and `@WizisCool/dsh-ears`, and it opens the GitHub repo. Also Host-read version, MIT, dsh rc.6/rc.7, and a click-only Host npm `latest` check. Notify + copy `dsh plugin --profile web update dsh-ears`. 404 is unpublished, not up-to-date. No install. Publish waits for "可以发".
- Validation: `pnpm check` passed; `pnpm test` passed twice (261/261 across 24 files); `pnpm build` passed (client bundles `@thesvg/react` GitHub mark).
- Unfinished: live GUI refresh of the About tab (Host restart for the `repository` wire fields); first public release waits for "可以发".
- Blocked: none.
- Next: restart `dsh web` and refresh to inspect the repo row; do not publish until the user says so.
- Commit: pending.
...
- Commit: `341df66 chore: keep the package root to package and build files`.

## Sleeping-branch review and split (2026-08-18)

- Completed: reviewed Host ASR/polish, settings auto-save, voice capture/session, Whisper lifecycle, and Host/Client Remote parity on `sleeping`. Fixed two shipped defects: the Host cloud model-list failure cache is now keyed by provider+credential (a corrected Groq key no longer reuses a 401 for 30s), and the composer no longer silently starts Web Speech for an unknown stored backend. Extracted independently testable units: `voice-capture.ts`, `settings-fields.ts`, `settings-locale.ts`, `whisper-discovery.ts`, `whisper-progress.ts`, `polish/route.ts`. Added shipped-function tests for unknown-backend gating, click/start/stop decisions, abandoned pending capture, Web Speech interim fallback, field validity, polish route selection, no-retry save, Whisper single-flight download, and shebang reads.
...
- Follow-up: glued `第一帮我…第二帮我…` still came back as one line because polish often never ran. The client skipped `dshEars/polish` when its local settings snapshot looked unconfigured, and the Host also returned immediately on an empty client provider/model pair. The client now always asks the Host after a transcript; the Host uses the stored route when polishing is enabled. Web Speech also commits the last interim text when the browser emits no final result. `pnpm check`, `pnpm test` (178/178), and `pnpm build` passed.
...
## Grouped cloud ASR settings record (2026-08-18)

- Completed: Host settings file now groups cloud ASR as `groq` / `customOpenAi` / `bailian` (each with `apiKey` and provider-specific model/endpoint/host). The plugin wire uses `cloudAsrGroqApiKey` instead of the ambiguous `cloudAsrApiKey`. A first `getSettings` rewrites a previous flat file. Per-provider models persist independently.
- Validation: `pnpm check` passed; `pnpm test` passed (196/196).
- Unfinished: Host restart required so the new schema writes the grouped file; the user's existing flat `dsh-ears` block is rewritten on first settings read.
- Blocked: none.
- Next: restart `dsh web`; do not push without authorization.
- Commit: this turn’s `feat` and `docs` commits.
...
- Completed: `qwen-audio-3.0-asr-flash` HTTP 400 on silent/too-short audio is treated as empty speech. Host returns `""`; the client also skips transcribe/polish for recordings under 400 ms or 512 bytes. Real 400s like `InvalidParameter: model not exist` still surface in red.
- Validation: `pnpm check` passed; `pnpm test` passed (209/209); `pnpm build` passed.
- Unfinished: Host + client need a `dsh web` restart and refresh. D-018 and live smokes remain pending.
- Blocked: none.
- Next: restart `dsh web` and refresh, then retry a silent Bailian click.
- Commit: this turn’s `fix` commit.
