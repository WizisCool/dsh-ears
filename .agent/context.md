...
A Host-side registry (`src/asr/providers.ts`) owns Groq, Deepgram, Bailian, Tencent Cloud, MiMo, and Custom:
...
- **Recognition**: grouped backend/provider menu (Local: Web Speech / Local Whisper; Cloud: Groq / Deepgram / Bailian / Tencent Cloud / MiMo / Custom), Whisper model lifecycle, Default/Vulkan/CUDA acceleration selector, and per-provider key/endpoint/host/model. Web Speech is the default backend for new or incomplete settings, so a fresh install works without a model download; Local Whisper remains selectable and its `default` acceleration is resolved by the Host from platform-supported and installed native variants. Each backend branch carries its own recognition-language row (none for Tencent Cloud; D-042): empty follows the dsh English/中文 locale for Web Speech and means automatic detection — the language parameter is omitted — for Local Whisper, Groq, Bailian, and custom OpenAI-compatible; Tencent Cloud keeps engine type as its language/engine selector with no separate row. Tencent Cloud and Deepgram expand to a service selector with standard recording and realtime recognition enabled. MiMo expands to an access-method selector (`api` or `token-plan`) with a regional cluster row for Token Plan. Changing acceleration after the first native load requires restarting the dsh Host.
