...
A Host-side registry (`src/asr/providers.ts`) owns Groq, Deepgram, Bailian, Tencent Cloud, MiMo, SiliconFlow, and Custom:
...
- SiliconFlow: OpenAI-compatible provider pinned to the domestic `https://api.siliconflow.cn/v1` base URL (D-045). Its `/models` listing passes `sub_type=speech-to-text`; its transcription endpoint is `POST {baseUrl}/audio/transcriptions` with a `SenseVoiceSmall` default model. Only the domestic edition is exposed; the international `api.siliconflow.com` is deferred until it exposes ASR models and is reachable with its own credential.

Each provider has its own write-only `role('secret')` key (D-023, D-032, D-040, D-043, D-044, D-045). The Host settings file groups them as `groq`, `deepgram`, `customOpenAi`, `bailian`, `tencent`, and `mimo`. The plugin wire uses explicit per-provider fields, including `cloudAsrDeepgramApiKey`, `cloudAsrTencentSecretKey`, and `cloudAsrMimoApiKey`; `getSettings` redacts secrets and reports configured booleans. `updateSettings` uses absent=keep / set / empty=clear per key. The browser never receives a key value. A first read rewrites a previous flat `cloudAsrApiKey` file into the grouped form.
...
- **Recognition**: grouped backend/provider menu (Local: Web Speech / Local Whisper; Cloud: Groq / Deepgram / Bailian / Tencent Cloud / MiMo / SiliconFlow / Custom), Whisper model lifecycle, Default/Vulkan/CUDA acceleration selector, and per-provider key/endpoint/host/model. Each backend branch carries its own recognition-language row (none for Tencent Cloud; D-042): empty follows the dsh English/中文 locale for Web Speech and means automatic detection — the language parameter is omitted — for Local Whisper, Groq, Bailian, SiliconFlow, and custom OpenAI-compatible; Tencent Cloud keeps engine type as its language/engine selector with no separate row. Tencent Cloud and Deepgram expand to a service selector with standard recording and realtime recognition enabled. MiMo expands to an access-method selector (`api` or `token-plan`) with a regional cluster row for Token Plan. SiliconFlow populates its model selector from the filtered `sub_type=speech-to-text` live list. Changing acceleration after the first native load requires restarting the dsh Host.
