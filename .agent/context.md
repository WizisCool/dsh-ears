...
Each provider has its own write-only `role('secret')` key (D-023, D-032, D-040, D-043, D-044, D-047). The Host settings file groups them as `groq`, `deepgram`, `customOpenAi`, `bailian`, `tencent`, `mimo`, and `siliconflow`. The plugin wire uses explicit per-provider fields, including `cloudAsrDeepgramApiKey`, `cloudAsrTencentSecretKey`, and `cloudAsrMimoApiKey`; `getSettings` redacts secrets and reports configured booleans. `updateSettings` uses absent=keep / set / empty=clear per key. The browser never receives a key value. A first read rewrites a previous flat `cloudAsrApiKey` file into the grouped form.
...
- **Recognition**: grouped backend/provider menu (Local: Web Speech / Local Whisper; Cloud: Groq / Deepgram / Bailian / Tencent Cloud / MiMo / SiliconFlow / Custom), Whisper model lifecycle, Default/Vulkan/CUDA acceleration selector, and per-provider key/endpoint/host/model. Web Speech is the default backend for new or incomplete settings, so a fresh install works without a model download; Local Whisper remains selectable and its `default` acceleration is resolved by the Host from platform-supported and installed native variants. Each backend branch carries its own recognition-language row (none for Tencent Cloud; D-042): empty follows the dsh English/中文 locale for Web Speech and means automatic detection — the language parameter is omitted — for Local Whisper, Groq, Bailian, and custom OpenAI-compatible; Tencent Cloud keeps engine type as its language/engine selector with no separate row. Tencent Cloud and Deepgram expand to a service selector with standard recording and realtime recognition enabled. MiMo expands to an access-method selector (`api` or `token-plan`) with a regional cluster row for Token Plan. Changing acceleration after the first native load requires restarting the dsh Host.
