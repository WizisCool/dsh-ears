...
const whisperLoading: WhisperModelView = { status: 'loading', state: { runtimeAvailable: false, downloaded: false, downloading: false, progress: null, bytes: null, totalBytes: null, error: null } }
...
describe('micUnavailableReason', () => {
  ...
  it('reports a downloading whisper model with its progress', () => {
    const ready: BackendState = { status: 'ready', backends: [backendInfo('local-whisper', true)] }
    expect(micUnavailableReason('local-whisper', ready, whisperView({ runtimeAvailable: true, downloaded: false, downloading: true, progress: 0.42, bytes: 1, totalBytes: 2, error: null })))
      .toEqual({ kind: 'model-downloading', percent: 0.42 })
  })

  it('reports a missing whisper model as the gray reason', () => {
    const ready: BackendState = { status: 'ready', backends: [backendInfo('local-whisper', true)] }
    expect(micUnavailableReason('local-whisper', ready, whisperView({ runtimeAvailable: true, downloaded: false, downloading: false, progress: null, bytes: null, totalBytes: null, error: null })))
  ...
  it('returns null when the whisper state query failed instead of graying on missing information', () => {
    const ready: BackendState = { status: 'ready', backends: [backendInfo('local-whisper', true)] }
    expect(micUnavailableReason('local-whisper', ready, whisperView({ runtimeAvailable: true, downloaded: false, downloading: false, progress: null, bytes: null, totalBytes: null, error: 'query failed' })))
      .toBeNull()
  })

  it('returns null for a downloaded whisper model', () => {
    const ready: BackendState = { status: 'ready', backends: [backendInfo('local-whisper', true)] }
    expect(micUnavailableReason('local-whisper', ready, whisperView({ runtimeAvailable: true, downloaded: true, downloading: false, progress: null, bytes: null, totalBytes: 100, error: null })))
      .toBeNull()
  })
  ...
  it('ignores whisper model state for non-whisper backends', () => {
    ...
    expect(micUnavailableReason('web-speech', ready, whisperView({ runtimeAvailable: true, downloaded: false, downloading: false, progress: null, bytes: null, totalBytes: null, error: null })))
      .toBeNull()
  })
})
