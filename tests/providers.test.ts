...
describe('cloud ASR provider registry', () => {
  ...
  it('registers unique provider ids with the shared protocol', () => {
    ...
    expect(cloudProviderEntry('volcengine')?.protocol).toBe('volcengine')
    ...
    expect(isKnownCloudProvider('volcengine')).toBe(true)
  ...
  it('pins the Volcano Engine dual-service preset with per-service resource ids', () => {
    const entry = cloudProviderEntry('volcengine')
    expect(entry?.protocol).toBe('volcengine')
    expect(entry?.storageKey).toBe('volcengine')
    expect(entry?.realtime).toBe(true)
    expect(entry?.realtimeServices).toEqual(['realtime'])
    expect(entry?.endpointEditable).toBe(false)
    expect(entry?.apiKeyRequired).toBe(true)
    expect(supportsModelListing('volcengine')).toBe(false)
    expect(isKnownCloudProvider('volcengine')).toBe(true)
    expect(cloudAsrBackendSelection('volcengine')).toEqual({ asrBackend: 'cloud-openai', cloudAsrProvider: 'volcengine' })
    expect(cloudAsrEndpointFor(settings({ cloudAsrProvider: 'volcengine' }))).toBe('https://openspeech.bytedance.com')
    const recordingSettings = settings({ asrBackend: 'cloud-openai', cloudAsrProvider: 'volcengine', cloudAsrVolcengineService: 'recording-file', cloudAsrVolcengineApiKey: 'vk' })
    expect(isCloudConfigurationValid(recordingSettings)).toBe(true)
    expect(isCloudAsrReady(recordingSettings)).toBe(true)
    expect(isCloudAsrRealtime(recordingSettings)).toBe(false)
    const realtimeSettings = settings({ asrBackend: 'cloud-openai', cloudAsrProvider: 'volcengine', cloudAsrVolcengineService: 'realtime', cloudAsrVolcengineApiKey: 'vk' })
    expect(isCloudAsrRealtime(realtimeSettings)).toBe(true)
    expect(cloudAsrModelFor(realtimeSettings)).toBe('volc.seedasr.sauc.duration')
    expect(cloudAsrCredentialFor(settings({ cloudAsrProvider: 'volcengine', cloudAsrVolcengineApiKey: ' vk ' }))).toBe('vk')
  })
