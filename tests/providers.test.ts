...
describe('cloud ASR provider registry', () => {
  ...
  it('keeps Tencent Cloud as one provider with a Flash Edition service default', () => {
    expect(cloudAsrEndpointFor(settings({ cloudAsrProvider: 'tencent' }))).toBe('https://asr.cloud.tencent.com/asr/flash/v1')
...
describe('cloud ASR runtime readiness', () => {
  ...
  it('requires Tencent Cloud credentials and an executable service', () => {
    expect(isCloudAsrReady(settings({ cloudAsrProvider: 'tencent', cloudAsrTencentService: 'flash', cloudAsrTencentAppId: '1250000000', cloudAsrTencentSecretId: 'AKID', cloudAsrTencentSecretKey: 'secret' }))).toBe(true)
    expect(isCloudAsrReady(settings({ cloudAsrProvider: 'tencent', cloudAsrTencentService: 'big-model', cloudAsrTencentAppId: '1250000000', cloudAsrTencentSecretId: 'AKID', cloudAsrTencentSecretKey: 'secret' }))).toBe(false)
  })
