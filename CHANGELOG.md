...
- Per-field validate-on-edit settings model (D-024): each edited field is validated alone on the auto-save cadence, red errors appear only for invalid user input and real failures, untouched fields never show errors, backend/provider switches save immediately, and an incomplete polishing pair leaves polishing dormant. Guidance prompts for "not yet configured" states are removed, and the first-load alert appears only after the automatic retry fails, with neutral wording.
