...
>
> The Recognition tab shows only acceleration variants supported by the current platform and installed native packages. The official macOS artifacts provide Default only, so CUDA is not shown there; Windows x64 and Linux options depend on the optional variants actually installed. Changing acceleration after the native runtime has loaded requires a dsh Host restart. The pinned `@fugood/whisper.node@1.1.2` Windows x64 CUDA binary requires the CUDA 12 `cudart64_12.dll` and `cublas64_12.dll`; when the runtime libraries do not match, settings marks that variant unavailable and the user can select another available backend. Model weights are downloaded into the local cache
