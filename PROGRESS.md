...
- Replaced the auto-save with the platform card's explicit save model (D-026), then restored a safer per-field auto-save (D-031): the card chrome and Save/Discard footer are gone; valid drafts flush after 400 ms, on blur, or when the section unmounts; invalid drafts are skipped locally; a Host rejection keeps the drafts and does not retry in a loop. Validation stays per-field and immediate; empty text clears a field on flush (the API key keeps absent=keep with a staged clear action, an emptied recording limit resets to the default), and a mid-save edit survives the in-flight write then flushes afterward.
