...
- Provider switches clear stale model/reasoning selections; the provider/model pair activates polishing, and an incomplete pair leaves polishing dormant (D-024 revises the pair rule; Host and client agree), and the reasoning picker offers the adapter's `off` effort as an explicit switch-off.
...
- Switched the settings page to a per-field validate-on-edit model (D-024): validation recomputes on the same 400 ms cadence as the auto-save, only for the field the user edited (touched gating — untouched fields are never marked invalid); an invalid draft skips only its own write while every valid value saves immediately, including backend/provider switches; red is reserved for invalid user input and real failures, and all "not yet configured" guidance prompts (backend-unavailable line, no-key model hint, amber incomplete hints) are gone — a fresh unconfigured page shows no prompts at all. The first-load alert appears only after the single automatic retry fails, with neutral wording. Enabling polishing saves at once; an incomplete provider/model pair leaves polishing dormant (revises the D3 pair rule). No third-party form library is adopted (D-025); the validation-timing semantics follow industry and dsh platform research recorded in `.agent/research/`.
- Replaced the auto-save with the platform card's explicit save model (D-026): edits stage as drafts and commit through the footer's separator + Save/Discard buttons (Save blocked unless dirty, valid, and idle; an invalid draft blocks the whole save and keeps the drafts; Host rejection keeps the drafts with the red saveFailed line; Discard drops everything without confirmation). Validation stays per-field and immediate; empty text clears a field on save (the API key keeps absent=keep with a staged clear action, an emptied recording limit resets to the default), and a mid-save edit survives the in-flight write. The Enter-to-save shortcut and the save-success flash were removed with the auto-save machinery.
- Removed the last Host-side cross-field gates from the settings registration validate (`5a8bb59`): the Groq key write is no longer rejected while the cloud model is still unselected, and a custom provider without an endpoint no longer blocks other field writes. Runtime readiness stays guarded by `transcribe()`'s own model/key/endpoint checks and the D-021 microphone gating.
...
- Reworked the Recognition selector into one grouped menu (Local: Web Speech / Local Whisper; Cloud providers: Groq / Custom OpenAI-compatible) with `MenuLabel`/`MenuSeparator` groups, per-provider hint text, a write-only API key row styled after the shipped SecretField (state rendered as the input placeholder, clear action kept), and a live-model row for Groq (empty until key, retry on failure, stale-model notice); preset endpoints stay pinned in the registry and are not displayed.
